...
function parseRssItems(text, { source, lang, max = 15, sourceFromFeed = false } = {}) {
  ...
  for (const raw of rawItems.slice(0, max)) {
    ...
    // Investing.com envia data GMT sem fuso ("YYYY-MM-DD HH:MM:SS"); normaliza para ISO UTC
    if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(pub)) pub = pub.replace(' ', 'T') + 'Z';
    let image = '';
...
async function fetchNews() {
  ...
  const sources = [
    // --- Português ---
    feed('https://www.infomoney.com.br/feed/', { source: 'InfoMoney', lang: 'pt', max: 12 }),
    feed('https://br.investing.com/rss/news.rss', { source: 'Investing.com', lang: 'pt', max: 10 }),
    feed('https://www.bloomberglinea.com.br/arc/outboundfeeds/rss/?outputType=xml', { source: 'Bloomberg Línea', lang: 'pt', max: 8 }),
    feed('https://news.google.com/rss/search?q=wall+street+OR+fed+OR+bolsas+globais+OR+economia+global+when:1d&hl=pt-BR&gl=BR&ceid=BR:pt-419', { source: 'Google News', lang: 'pt', max: 10, sourceFromFeed: true }),
    // --- English ---
  ...
  // sort by pubDate (newest first) and interleave PT / EN so both languages show up early
  const newsTime = (n) => {
    const t = Date.parse(n.pubDate || '');
    return isNaN(t) ? 0 : t;
  };
  const interleave = (arr) => {
    const byDate = (a, b) => newsTime(b) - newsTime(a);
    const pt = arr.filter(i => i.lang === 'pt').sort(byDate);
    const en = arr.filter(i => i.lang === 'en').sort(byDate);
    const out = [];
